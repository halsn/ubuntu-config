...
bootstrap() {
  echo "-----------bootstrap-------------"
  sudo apt install -y ppa-purge tree time curl wget gawk wordnet entr inotify-tools silversearcher-ag htop ncdu exuberant-ctags nyancat vim w3m ripgrep tig icdiff lshw gcc g++
  echo "------------finished-------------"
  echo ""
}
...
config_coc() {
  ...
  extensions="coc-tsserver coc-tslint coc-eslint coc-json coc-html coc-css coc-ultisnips coc-dictionary coc-lists coc-word coc-python coc-diagnostic"
